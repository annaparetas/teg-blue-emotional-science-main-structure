...
def main() -> int:
    ...
    groups = [{"id": "body", "name": "Body-condition signals", "question": "What internal condition is the organism regulating?",
              ...
              {"id": "emotion", "name": "Emotion signals", "question": "What does this present or represented condition mean for the organism?",
               "families": []}]
    rationales = {key: clean(value) for key, value in re.findall(r'<p data-family-rationale="([^"]+)">(.*?)</p>', CANON.read_text(encoding="utf-8"), flags=re.S)}
    used = set()
    ...
    for group in groups:
        for family in group["families"]:
            family["rationale"] = rationales[family["id"]]
            family["explanation"] = f'index.html#family-meaning-{family["id"]}'
    ...
    for sid, rec in signals.items():
        family = next(f for g in groups for f in g["families"] if f["id"] == rec["family"])
        rec["family_rationale"] = family["rationale"]
        rec["family_explanation"] = family["explanation"]
        f = rec["fluid"]
    ...
    for sid, rec in signals.items():
        ...
        record_lines += ["### Why this family?", "", rec["family_rationale"], "", f'[Group → family → signal]({rec["family_explanation"]})', ""]
        if sid == "love":
